...
/* =   저작권			:	(주)다우기술      	            = */
/* ========================================================================== */
function MobileCancel(  $server_ip, $port, $cpid, $enckey, $timeout ) {
	...
	$sendBody	= _MaxLength($daoutrx, 20).'|'._MaxLength($amount, 10).'|'._MaxLength($cancelmemo, 50);
...
function _packet_encrypt($param_input_string, $enckey)
{
	...
	$cipher_txt = @mcrypt_encrypt(MCRYPT_RIJNDAEL_128, hex2bin(_hash($suq).$key2), $padding_string, MCRYPT_MODE_CBC,_unhash($xst));
...
{  
	...
	$key2 = bin2hex($enckey);
    	$decrypttext = @mcrypt_decrypt(MCRYPT_RIJNDAEL_128, hex2bin(_hash($qus).$key2), hex2bin($param_input_string), MCRYPT_MODE_CBC,_unhash($edf));
