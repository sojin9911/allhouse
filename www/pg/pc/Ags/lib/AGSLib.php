...
class agspay40
{
	...
	*/
	function startPay(  )
	{
		...
		if( strlen($this->ERRMSG) == 0 ){
			...
			/* Make Pay Msg */
			if( ($MakeMsgResult = $this->Makeerror_msgall()) != true){
			...
			/* Send & Recv Msg */				
			if( ($ParseResult = $this->SendRecverror_msgall()) != true){
			...
			/* RecvMsg Parsing */
			if( ($ParseResult = $this->Parseerror_msgall()) != true){
	...
	*/
 	function Makeerror_msgall() 
	{
		...
		****************************************************************************/
		...
		return true;
	}	// Makeerror_msgall() End


	/*
		Make Cancel Msg                       
	*/
 	function MakeCancelerror_msgall() 
	{
	...
	*/
	...
	}	//Parseerror_msgall() End


	/*
		RecvCancelMsg Parsing                       
	*/
 	function ParseCancelerror_msgall() 
	{
	...
	*/
 	function NetCancel() 
	{
		...
		if( $this->REQUEST["UseNetCancel"] == "true" || $this->REQUEST["Type"] == "Cancel" )
		{
			...
			if( !($this->MakeCancelerror_msgall()) )
			{
				$this->log->WriteLog( ERROR, "Make CancelMsg Error");
				return false;
			}
			if( !($this->SendRecverror_msgall()) )
			{
				$this->log->WriteLog( ERROR, "Send & Recv Msg Error");
				return false;
			}
			if( !($this->ParseCancelerror_msgall()) )
			{
	...
	*/
 	function checkPayResult( $TID ) 
	{
		...
		****************************************************************************/
		...
		if( !($this->SendRecverror_msgall()) )	//소켓을 통해 거래결과 확인
		{
	...
	*/
	...
	/*
		Set Error Msg ;
	*/
	function SetErrorerror_msgall($rSuccYn,$rResMsg)
	{	
	...
	*/
	...
	/* 
		경고 메세지
	*/
	function Alerterror_msgall( $msg , $go=0)
	{
