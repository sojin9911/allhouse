...
class INIpay50
{
	...
	function startAction()
	{
  	...
  	if( $this->m_type == TYPE_SECUREPAY   || $this->m_type == TYPE_FORMPAY  || $this->m_type == TYPE_OCBSAVE  		|| 
      	$this->m_type == TYPE_AUTHBILL 		|| $this->m_type == TYPE_FORMAUTH || $this->m_type == TYPE_REQREALBILL	|| 
				$this->m_type == TYPE_REPAY    		|| $this->m_type == TYPE_RECEIPT	|| $this->m_type == TYPE_AUTH        
    )
		{
		...
		/*--------------------------------------------------*/
		//폼페이 암호화( formpay, cancel, repay, recept )
		/*--------------------------------------------------*/
		if( $this->m_type == TYPE_CANCEL	|| $this->m_type == TYPE_REPAY		|| 
			  $this->m_type == TYPE_FORMPAY	|| $this->m_type == TYPE_RECEIPT 	|| 
				$this->m_type == TYPE_CAPTURE || 
  	...
  	if( $this->m_type == TYPE_SECUREPAY   || $this->m_type == TYPE_FORMPAY  || $this->m_type == TYPE_OCBSAVE  || 
      	...
      	$this->m_type == TYPE_REQREALBILL || $this->m_type == TYPE_REPAY    || $this->m_type == TYPE_RECEIPT	||
      ...
      ( $this->m_type == TYPE_SECUREPAY || $this->m_type == TYPE_OCBSAVE || 
		...
		//2008.04.01
		/*--------------------------------------------------*/
		if( $this->GetResult(NM_RESULTCODE) == "01" && $this->m_type == TYPE_CANCEL && $this->m_cancelRC == 0 )
		{
	...
	function MakeChkFake()
	{
      ...
      foreach ($this->m_REQUEST as $key => $val)
      {
			...
			$this->m_Log->WriteLog( INFO, "CHKFAKE KEY MAKE OK" );
	...
	function NetCancel()
	{
     	...
     	if ( $this->m_type == TYPE_CANCEL || $this->m_type == TYPE_REPAY || $this->m_type == TYPE_TYPE_RECEIPT || 
