	...
	<div class="layout_fix">
		...
		<!-- ◆마이페이지 주문통계 -->
			...
			<div class="mypage_info">
				...
				<div class="shop_info hide">
					...
					<!-- 내역 없을때 0으로 표기 -->
					<div class="info_box">
						<div class="txt_box"><span class="sub_tit le_2">주문</span><span class="txt">진행중인 주문 <strong><?php echo number_format(get_order_ing_cnt(array('접수대기', '접수완료', '구매발주', '배송중'))); ?>건</strong></span></div>
		...
		<div class="c_mypage_total hide">
			<ul>
				<li>
					<div class="inner_box">
						<div class="icon"><img src="<?php echo $SkinData['skin_url']; ?>/images/c_img/my_total_wait.gif" alt="접수대기"></div>
						<div class="txt">접수대기</div>
						<!-- 내역 없으면 0으로 표기 -->
						<div class="total_num"><?php echo number_format($order_status['접수대기']); ?></div>
					</div>
				</li>
				<li>
					<div class="inner_box">
						<div class="icon"><img src="<?php echo $SkinData['skin_url']; ?>/images/c_img/my_total_complete.gif" alt="접수완료"></div>
						<div class="txt">접수완료</div>
						<div class="total_num"><?php echo number_format($order_status['접수완료']); ?></div>
					</div>
				</li>
				<li>
					<div class="inner_box">
			...
			<div class="order_search">
				<ul>
					...
					<li class="period">
						...
						<!--3개월 코드로 복사했습니다-->
						<a href="/<?php echo URI_Rebuild('?', array('pn'=>'mypage.order.list', 'date'=>'year', 'o_status'=>$o_status)); ?>" class="btn<?php echo ($date == 'year' ? ' hit' : null); ?>">1년</a>
						</div>
					</li>
					<!-- 날짜선택 / 날짜선택시 달력 노출 -->
					<li class="date">
					...
					<li class="state_select hide">
						<select name="o_status">
							...
							<option value="접수대기" <?php echo ($o_status == '접수대기' ? 'selected' : null); ?>>접수대기</option>
							<option value="접수완료" <?php echo ($o_status == '접수완료' ? 'selected' : null); ?>>접수완료</option>
		...
		<div class="c_order_list">
					<p class="c_order-tit">주문목록 / 배송조회 내역 총 <span><?php echo number_format($TotalCount);?></span>건</p>
					<?php
						// 주문내역이 있을때
						if(count($res) > 0 ) {
					?>
							...
							<?php
								foreach($res as $k=>$v){
									...
									if($thumb_img=='') $thumb_img = $SkinData['skin_url']. '/images/skin/thumb.gif';
										...
										if( in_array($v['o_status'] , array('접수대기','접수완료','접수대기')) ){

											if($v['o_status']!='접수대기'&&($v['o_paymethod']=='virtual'||$v['o_paymethod']=='online')) { // 주문상태가 접수대기가 아니고, 결제방법이 가상,무통장 인건만
									...
									switch($v['o_status']){
										case '구매발주':
										case '접수완료':
											$o_status_print = '<span class="icon complete">접수완료</span>';
										break;

										case '접수대기':
											$o_status_print = '<span class="icon wait">접수대기</span>';
										break;
