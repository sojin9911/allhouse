<div class="c_section c_mypage js_guest_order">
	<div class="layout_fix">
		<!-- ◆공통페이지 타이틀 -->
		<div class="c_page_tit">
			<!-- 마이페이지 메인으로 이동 -->
			<div class="title">비회원주문조회</div>
			<!-- 로케이션 -->
		...
		?>
						...
						<table>
							...
							<?php
								...
								foreach( $sres as $k=>$v ){
										...
										if($v['o_paystatus']=='Y' && $sv['op_is_addoption']!='Y' && $sv['op_cancel'] == 'N' && $row['o_canceled'] == 'N'){ // SSJ : 취소된 주문은 버튼이 노출되지 않도록 수정 : 2021-04-14
											switch($sv['op_sendstatus']) {
                                                ...
                                                case "배송중":
                                                    ...
                                                    // ![LCY] 2020-07-13 -- 네이버페이 사용자 주문취소 비활성 패치 :: $row['npay_order'] != 'Y'  --
                                                    if(!$sv['op_complain'] && $row['npay_order'] != 'Y' ){
									...
									else if($op_status['del_ready']>0){ // 결제완료가 하나라도 있으면 결제완료상태 :: [결제완료, 배송완료, 주문취소] - [결제완료]
							...
							?>
									<tr>
										...
										<td>
											...
											<div class="thumb_box">
												<a href="<?php echo $p_url; ?>" target="_blank"><img src="<?php echo $p_thumb; ?>" alt="<?php echo addslashes($p_name); ?>"  id="test"></a>
											</div>
					...
					<table>
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									<?php echo ($row['o_content']?nl2br(htmlspecialchars($row['o_content'])):''); ?>
								</td>
							</tr>
						</tbody>
					</table>
				...
				<div class="c_btnbox ">
					<ul>
						<li><a href="#none" class="c_btn h55 black line js_search_clear">초기화</a></li>
...
<script>
	...
	// 검색 초기화
	$(document).on('click', '.js_search_clear', function(){
		document.location.href = '/?pn=member.login.form&_rurl=<?php echo urlencode($_rurl); ?>';
	});
</script>
