	...
	<table>
		...
		<tbody>
			...
			foreach( $res as $k=>$v ){
				...
				if($v['pt_inid'] == get_userid())	$qna_btn = '<div><a href="#none" onclick="qna_del(\''.$v['pt_uid'].'\');" class="c_btn h22 line">삭제</a></div>';
				$qna_img = $v['pt_img'] ? '<div class="photo"><img src="'. IMG_DIR_PRODUCT.$v['pt_img'] .'" alt=""></div>' : '';

				// 리플 추출
				$reply_icon = '<span class="c_tag h22 light line">답변대기</span>';
				...
				foreach($reply_r as $kk=>$vv) {
					...
					$reply_icon = '<span class="c_tag h22 black line">답변완료</span>';
			...
			?>
					<tr class="qna_box_area" id="<?php echo $qna_content_uid; ?>" data-hit="false">
						...
						<td class="title">
							<div class="posting">
								<a href="#none" onclick="qna_show('<?php echo $qna_content_uid; ?>'); return false;" class="upper_link" title="<?php echo $qna_title; ?>"><img src="<?php echo $SkinData['skin_url']; ?>/images/skin/blank.gif" alt=""></a>
								<!-- 아이콘 -->
								<span class="icon">
									<?php echo $new_icon; ?>
								</span>
						...
						<td class="name"><?php echo $qna_id; ?></td>
						<td><span class="date"><?php echo $qna_rdate; ?></span></td>
						<!-- 클릭 후 내용닫기로 타이틀 변경 -->
						<td><a href="#none" onclick="qna_show('<?php echo $qna_content_uid; ?>'); return false;" title="내용열기" class="arrow"></a></td>
					</tr>
					<tr class="board_box">
						<td colspan="6">
							<!-- 타이틀 -->
							<span class="title"><?php echo $qna_title; ?></span>
							<!-- 첨부이미지 -->
							<?php echo $qna_img; ?>
							<!-- 내용 -->
		...
		<span class="lineup">
			<span class="nextprev">
				...
				<span class="btn<?php echo ($listpg == $first ? '' : ' click');?>">
					...
					<a href="#none" onclick="qna_view(<?php echo $first; ?>); return false;" title="처음페이지" class="ok"><span class="icon ic_first"></span></a>
				</span>
				<span class="btn<?php echo ($listpg == $prev ? '' : ' click');?>">
					<span title="이전페이지" class="no"><span class="icon ic_prev"></span></span>
					<a href="#none" onclick="qna_view(<?php echo $prev; ?>); return false;" title="이전페이지" class="ok"><span class="icon ic_prev"></span></a>
				</span>
			</span>
			...
			<span class="number">
				...
				for($ii=$for_start_num;$ii<=$for_end_num;$ii++) {
					if($ii != $listpg)	echo '<a href="#none" onclick="qna_view('.$ii.')" class="">'. $ii .'</a>';
			...
			<span class="nextprev">
				...
				<span class="btn<?php echo ($listpg == $next ? '' : ' click');?>">
					...
					<a href="#none" onclick="qna_view(<?php echo $next; ?>); return false;" title="다음페이지" class="ok"><span class="icon ic_next"></span></a>
				</span>
				<span class="btn<?php echo ($listpg == $last ? '' : ' click');?>">
					<span title="마지막페이지" class="no"><span class="icon ic_last"></span></span>
					<a href="#none" onclick="qna_view(<?php echo $last; ?>); return false;" title="마지막페이지" class="ok"><span class="icon ic_last"></span></a>
				</span>
			</span>
		</span>
	</div>
