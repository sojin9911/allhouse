...
<div class="c_section">
	...
	<div class="c_attend ">
		<?php if(!$event_trigger) { ?>
			...
			<!-- [모바일] 이벤트별 배너 (1000 x free) / 없으면 PC이미지불러옴  -->
			<?php if($ready_img <> ''){ ?>
		...
		<?php }else{ ?>
			...
			<div class="attend_top">
				<ul>
					<li class="year_box">
						<div class="year">
							<div class="num"><strong><a href="/?pn=promotion.attend&selected=<?php echo $today_date; ?>"><?php echo $selected_year; ?>.<?php echo $selected_month; ?></a><!-- 여기클릭하면 오늘날짜로 --></strong></div>
							<!-- 이전,다음버튼 -->
							<?php if($prev_month){ ?>
					...
					<!-- 클릭시 btn클래스에 hit 추가 / '오늘 출석체크 완료!'로 변경 -->
					<?php if($today_status === true) { ?>
						<li class="attend_btn "><a href="#none" class="btn hit"><span class="txt">오늘 출석체크 완료!</span></a></li>
					<?php } else { ?>
						<li class="attend_btn "><a href="#none" onclick="checkin(); return false;" class="btn "><span class="txt">오늘 출석체크 하기!</span></a></li>
					<?php } ?>
				</ul>
			</div>
