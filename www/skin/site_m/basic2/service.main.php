...
<div class="c_section c_cs_main">
					...
					<div class="input_box">
						<div class="search_box">
							<form name="faqSearch" method="get">
								...
								<input type="search" class="input_search" name="searchWord" placeholder="궁금하신 점을 먼저 검색할 수 있습니다."/>
								<input type="submit" class="btn search_btn" value="질문검색"/>
							</form>
						</div>
					</div>
					<?php
		...
		<div class="c_faq_best">
			<div class="faq_list">
				...
				?>
				<ul class="js_faq_list_item" data-uid="<?php echo $v['bf_uid'] ?>">
					<li class="num"><?php echo $_num; ?></li>
					<li class="field"><?php echo $_type; ?></li>
					<li class="tit">
						<div class="tit_box">
							<a href="#none" onclick="return false" data-uid="<?php echo $v['bf_uid']; ?>" class="upper_link js_open_faq_content" title="<?php echo $_title; ?>"></a>
							<!-- 아이콘 -->
							<span class="icon">
								<?php echo count($arrIcon) > 0 ? implode("",$arrIcon): null;?>
							</span>
							<span class="tt"><?php echo $_title; ?></span>
						</div>
					</li>
					<li class="t_arrow"><a href="#none" onclick="return false;" title="내용열기" data-uid="<?php echo $v['bf_uid']; ?>" class="btn js_open_faq_content"></a></li>
				</ul>
				<!-- 자주묻는질문박스 클릭시 노출 -->
				<ul class="js board_box" style="display: none;" data-uid="<?php echo $v['bf_uid'] ?>">
					<li>
			...
			<div class="new_post ">
				...
				<?php if( count($resPost[$k]) > 0) { ?>
					...
					?>
						<li>
							<div class="posting">
								<a href="/?pn=board.view&_uid=<?php echo $sv['b_uid'] ?>&_menu=<?php echo $sv['b_menu']; ?>" class="upper_link" title="<?php echo $_title;?>"></a>
								<!-- 이벤트리스트에서는 이벤트기간 노출 -->
				...
				<div class="faq_box">
					...
					<div class="c_btnbox">
						<ul>
							...
							<li><a href="/?pn=mypage.inquiry.form" class="c_btn h55 color<?php echo is_login() == false ? ' js_login':'' ?>">1:1 온라인문의</a></li>
