...
<?php }else {  ?>
	...
	<?php foreach($listPost as $k=>$v) {  ?>
		...
		<ul>
			...
			<li class="t_writer"><?php echo $v['writer']; ?></li>
			<li class="date"><?php echo $v['rdate']; ?></li>
			<li class="t_hit"><?php echo $v['hit']; ?></li>
			<li class="state">
			<?php if( $v['replayStatus'] == 'Y'){ // 답변완료?>
				<span class="c_tag h22 red line">답변완료</span>
			<?php }else if( $v['replayStatus'] == 'R') { // 답변대기 ?>
