...
<form name="frm" action="_product.guide.pro.php" method="post" enctype="multipart/form-data" autocomplete="off">
	...
	<?php if($SubAdminMode !== true) { ?> <input type="hidden" name="g_user" value="<?php echo ($r['g_user']?$r['g_user']:'_MASTER_'); ?>"> <?php } ?>
			...
			<tbody>
				...
				<tr>
					...
					<td>
							...
							<?php } ?>
							<?php echo _InputSelect('g_user', array_keys($arr_customer), $r['g_user'], ' class="select2"  ', array_values($arr_customer), ''); ?>
					</td>
				</tr>
				<?php } ?>
				<tr>
