...
// ------- 회원 적립금 분석 - 날짜별 목록 -------
	...
	foreach($mem_date as $k=>$v){
		...
		$arr_data['pay_ing'][str_replace("-" , "" , $v['rdate'])] = $v['pay_ing'];
		$app_max['pay_ing'] = ($app_max['pay_ing'] < $v['pay_ing'] ? $v['pay_ing'] : $app_max['pay_ing']);

		$arr_data['pay_end'][str_replace("-" , "" , $v['rdate'])] = $v['pay_end'];
		$app_max['pay_end'] = ($app_max['pay_end'] < $v['pay_end'] ? $v['pay_end'] : $app_max['pay_end']);

		$arr_data['use_end'][str_replace("-" , "" , $v['rdate'])] = $v['use_end'];
		...
		$arr_tot_cumul[end(explode("-" , $v['rdate']))] = array(
			...
			'cnt' => $arr_tot_cumul[end(explode("-" , $v['rdate']))]['cnt'] + $v['total_sum'] // 총액
		);
...
for($i=1 ; $i<=12 ; $i++){
	...
	$arr_date_num['pay_ing'][$i] = $arr_data['pay_ing'][$Select_Year . sprintf("%02d" , $i)] * 1;// 선택월 지급예정액
	$arr_date_num['pay_end'][$i] = $arr_data['pay_end'][$Select_Year . sprintf("%02d" , $i)] * 1;// 선택월 지급액
	$arr_date_num['use_end'][$i] = $arr_data['use_end'][$Select_Year . sprintf("%02d" , $i)] * 1;// 선택월 사용액
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 적립금 분석 Summary ------- ?>
				<div class="group_title"><strong><?php echo $Select_Year; ?>년 회원 적립금 분석 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('point_month_search');
	...
	var chartData = {
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 지급예정액',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 지급액',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 사용액',
		...
		{
			type: 'line',
			label: '<?php echo $Select_Year; ?>년 지급(예정)/사용액',
...
<?php
	...
	foreach( $arr_tot_cumul as $mem_datek => $mem_datev ){
		...
		$app_date = $Select_Year ."년 ". sprintf("%02d" , $mem_datek)."월";
		$app_date_key = $Select_Year . sprintf("%02d" , $mem_datek);
