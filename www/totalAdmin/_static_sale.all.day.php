...
$Select_Month = date('m', strtotime($pass_date));
	...
	foreach( $res as $k=>$v ){
		...
		$arr_data['mobileY'][DATE("Ymd" , strtotime($v['rdate']))] = $v['mobileY_sum_price_real'];
		$app_max['mobileY'] = ($app_max['mobileY'] < $v['mobileY_sum_price_real'] ? $v['mobileY_sum_price_real'] : $app_max['mobileY']);

		$arr_data['mobileN'][DATE("Ymd" , strtotime($v['rdate']))] = $v['mobileN_sum_price_real'];
		...
		$arr_tot_cumul[DATE("d" , strtotime($v['rdate']))] = array(
			...
			'cnt' => $arr_tot_cumul[DATE("d" , strtotime($v['rdate']))]['cnt'] + $v['mobileY_sum_price_real'] + $v['mobileN_sum_price_real'] // 누적수
		);
	...
	# Chart 그래프 적용
	$arr_date_num = array(); // 일자별 매출
	$arr_date_date = array(); // 일자
	...
	for($i=1 ; $i<=date("t" , strtotime(date("{$Select_Year}-{$Select_Month}-01"))) ; $i++){
		...
		$arr_date_num['mobileN'][$i] = $arr_data['mobileN'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 PC 매출
		$arr_date_num['mobileY'][$i] = $arr_data['mobileY'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 모바일 매출
		...
		// PC - 최대값 체크 
		$arr_maxmin['mobileN']['max'] = ( $arr_maxmin['mobileN']['max']['cnt'] < $arr_date_num['mobileN'][$i] ? array('cnt'=>$arr_date_num['mobileN'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['mobileN']['max']);

		// MOBILE - 최대값 체크 
		$arr_maxmin['mobileY']['max'] = ( $arr_maxmin['mobileY']['max']['cnt'] < $arr_date_num['mobileY'][$i] ? array('cnt'=>$arr_date_num['mobileY'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['mobileY']['max']);



		// PC - 등록되어 있지 않은 경우 무조건 등록
		if(!isset($arr_maxmin['mobileN']['min']['cnt'])) {
			$arr_maxmin['mobileN']['min'] = array('cnt'=>$arr_date_num['mobileN'][$i] , 'date'=>sprintf("%02d" , $i));
		...
		// PC - 최소 정보일 경우 현시간 제외
			...
			$arr_maxmin['mobileN']['min'] = ( $arr_maxmin['mobileN']['min']['cnt'] > $arr_date_num['mobileN'][$i] ? array('cnt'=>$arr_date_num['mobileN'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['mobileN']['min']);
			$arr_avg['mobileN']['idx'] ++; // 계산할 횟수
			$arr_avg['mobileN']['cnt'] += $arr_date_num['mobileN'][$i]; // 계산할 접속수
		}
		...
		if(!isset($arr_maxmin['mobileY']['min']['cnt'])) {
			$arr_maxmin['mobileY']['min'] = array('cnt'=>$arr_date_num['mobileY'][$i] , 'date'=>sprintf("%02d" , $i));
			$arr_avg['mobileY']['idx'] ++; // 계산할 횟수
			$arr_avg['mobileY']['cnt'] += $arr_date_num['mobileY'][$i]; // 계산할 접속수
		}
		// MOBILE - 최소 정보일 경우 현시간 제외
		else if(date("Ymd") > $Select_Year . $Select_Month . sprintf("%02d" , $i)){
			$arr_maxmin['mobileY']['min'] = ( $arr_maxmin['mobileY']['min']['cnt'] > $arr_date_num['mobileY'][$i] ? array('cnt'=>$arr_date_num['mobileY'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['mobileY']['min']);
			$arr_avg['mobileY']['idx'] ++; // 계산할 횟수
			$arr_avg['mobileY']['cnt'] += $arr_date_num['mobileY'][$i]; // 계산할 접속수
		}
		// ------------------------ 가입기기별 ------------------------
	...
	<tbody>
		<tr>
			<td >
				...
				<? // ------- 매출 Summary ------- ?>
				<div class="group_title"><strong><?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 매출 요약</strong></div>
...
<script>
	...
	var chartData = {
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 PC 매출',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 모바일 매출',
		...
		{
			type: 'line',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 매출',
