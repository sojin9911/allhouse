			...
			<table class="table_list">
				...
				<thead>
					<tr>
						...
						<th scope="col">상품정보</th>
						<!-- LDD: 2019-01-18 네이버페이 패치 -->
							<th scope="col">N 주문번호</th>
							<th scope="col">N 상품주문번호</th>
				...
				<tbody>
					...
					foreach($OtherOrder as $ook=>$oov) {
						...
						// LDD: 2019-01-18 네이버페이 패치
						if(in_array($oov['npay_status'], array('PAYED', 'PLACE', 'DISPATCHED', 'CANCELED')) === false) {
							$StatusIconSub = '<span class="c_tag gray h22 t4" style="width:auto; padding:0 5px!important">'.$StatusArray[$oov['npay_status']].'</span>';
						}
					...
					?>
						<tr>
							...
							<!-- LDD: 2019-01-18 네이버페이 패치 -->
								<td<?php echo $TDcolor; ?>><?php echo ($oov['npay_order_group']?$oov['npay_order_group']:'이전주문'); ?></td>
								<td<?php echo $TDcolor; ?>><?php echo ($oov['npay_order_code']?$oov['npay_order_code']:'연동대기'); ?></td>
								<td<?php echo $TDcolor; ?>>
									<div class="lineup-vertical">
										<?php echo (!$oov['npay_order_group']?$SyncIcon['이전주문']:$SyncIcon[$oov['npay_sync']]); ?>
									</div>
								</td>
							<!-- LDD: 2019-01-18 네이버페이 패치 -->
							<td<?php echo $TDcolor; ?>>
					...
					<tr>
						...
						<td>
							<?php if($r['npay_status'] == 'PAYED') { ?>
								<a href="/addons/npay/npay.order.pro.php?path=<?php echo $AdminPath; ?>&_mode=PlaceProductOrder&_uid=<?php echo $_uid; ?>&code=<?php echo $r['npay_order_code']; ?>&_PVSC=<?php echo $_PVSC; ?>" class="c_btn h27 purple line bold">발주처리 하기</a>
							<?php } else { ?>
								<span class="c_tag h27 gray bold">발주처리 됨</span>
							<?php } ?>
							<div class="tip_box">
						...
						<td<?php echo (!in_array($r['npay_status'], array('PAYED', 'PLACE'))?' colspan="3"':null); ?>>
							<div class="lineup-resposive">
								...
								<?php } else { ?>
									...
									<a href="<?php echo ($NPayCourier[$r[op_sendcompany]]?$NPayCourier[$r[op_sendcompany]]:$arr_delivery_company[$r[op_sendcompany]]).rm_str($r['op_sendnum']); ?>" class="c_btn h22 green line h22 t4" target="_blank">배송조회</a>
								<?php } ?>
							</div>
							<?php if(!$_expressname) { ?>
							<div class="tip_box">
