...
foreach($mem_date as $k=>$v){
	...
	$arr_data['in_join_ua_PC_plus'][DATE("Ymd" , strtotime($v['rdate']))] = $v['in_join_ua_PC_plus'];
	...
	$arr_data['in_join_ua_MOBILE_plus'][DATE("Ymd" , strtotime($v['rdate']))] = $v['in_join_ua_MOBILE_plus'];
...
for($i=1 ; $i<=date("t" , strtotime(date("{$Select_Year}-{$Select_Month}-01"))) ; $i++){
	...
	$arr_date_num['in_join_ua_PC_plus'][$i] = $arr_data['in_join_ua_PC_plus'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 PC 회원수
	$arr_date_num['in_join_ua_MOBILE_plus'][$i] = $arr_data['in_join_ua_MOBILE_plus'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 MOBILE 회원수
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 가입형태 분석 Summary ------- ?>
				<div class="group_title"><strong><?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 회원 가입형태 분석 요약</strong></div>
...
<script>
	...
	var chartData = {
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 가입기기 - PC 수',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 가입기기 - MOBILE 수',
	...
	var config2 = {
		...
		options: {
			responsive: true,
			legend: {position: 'top',},
			title: {display: true,text: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 회원구분'},
			animation: {
				animateRotate: false,
				animateScale: true
			}
    ...
    var config3 = {
		...
		options: {
			responsive: true,
			legend: {position: 'top',},
			title: {display: true,text: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 수신여부'},
			animation: {
				animateRotate: false,
				animateScale: true
			}
...
<script type="text/javascript" class="code-js">
    ...
    var grid = new tui.Grid({
        ...
        columnModelList: [
            {"title" : "<b>날짜</b>", "columnName" : "rdate", "align" : "center", "width" : 60 },
