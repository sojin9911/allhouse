...
switch( $pass_mode ){
    ...
    case "1depth_del":
        ...
        if(in_array($pass_mode , array("1depth_del" , "2depth_del"))) {
            // 삭제전 하위정보 확인
            $ique = " select count(*) from smart_common_option where co_suid='".$pass_common_uid."' and find_in_set('{$pass_uid}' , co_parent) > 0 and co_depth <= '". (rm_str($pass_mode)+1) ."' "; // 1차 정보
        ...
        _MQ_noreturn(" update smart_common_option set co_sort=co_sort-1  where co_suid='".$pass_common_uid."' and co_depth='". $r['co_depth'] ."' and co_parent='" . $r['co_uid'] . "' and co_sort > '". $r['co_sort'] ."' ");
        ...
        // 삭제
        mysql_query("delete from smart_common_option where co_uid='{$pass_uid}'");
    break;
    ...
    case "3depth_add":
        ...
        $ique = " select * from smart_common_option where co_suid='".$pass_common_uid."' and co_uid='{$pass_uid}' "; // 2차 정보
        $ires = mysql_query($ique);
        $ir = mysql_fetch_assoc($ires);

        // 순번추출
        $r = _MQ(" select ifnull(max(co_sort),0) as max_sort from smart_common_option where co_suid='".$pass_common_uid."' and co_depth='3' and co_parent='" . $ir['co_parent'].",".$ir['co_uid'] . "' ");
        ...
        mysql_query("
            insert smart_common_option set
                ...
                co_poptionname='',
                co_depth='3',
                co_parent='{$ir['co_parent']},{$ir['co_uid']}',
                co_sort='". $max_sort ."'
        ");
        break;
    ...
    case "2depth_add":
        ...
        mysql_query("
            insert smart_common_option set
                ...
                co_poptionname='',
                co_depth='3',
                co_parent='{$ir['co_uid']},{$uid_2depth}',
                co_sort='". $max_sort3 ."'
        ");
     break;
		...
		<table class="table_list">
			...
			<tbody>
				...
				<tr class="option_depth1">
					...
					<td>
						<input type="text" name="co_info[<?=$r['co_uid']?>][co_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?=$r['co_poptionname']?>" style="width:<?=($app_option1_type == 'color' ? '300px;' : '100%')?>" />
						<?php
							// ----- 컬러형일 경우 -----
				...
				<?php
					...
					if(sizeof($res2) <= 0){
						echo '
							<tr>
								<td colspan="4">
									<!-- 내용없을경우 --><div class="common_none" style="margin:40px 0 35px 0;;"><div class="no_icon"></div><div class="gtxt">등록된 옵션정보가 없습니다.</div></div>
								</td>
							</tr>
						';
					}
				...
				?>
					<!-- 2차인경우 -->
					<tr>
						<td class="option_depth2"><!-- 2차인 경우 클래스값 option_depth2 -->
							<span class="fr_bullet">2차</span>
						</td>
						<td>
							<input type="text" name="co_info[<?php echo $r2['co_uid']; ?>][co_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo $r2['co_poptionname']; ?>" style="width:<?=($app_option2_type == 'color' ? '300px;' : '100%')?>" />
							<?php
								// ----- 컬러형일 경우 -----
						...
						<td>
							<div class="lineup-center">
								...
								<a href="#none" onclick="category_apply_save('3depth_add', '<?php echo $r2['co_uid']; ?>'); return false;" class="c_btn h22 ">추가</a><!-- 끼워넣기 추가버튼 -->
					...
					<tr>
						<td class="option_depth3"><!-- 3차인 경우 클래스값 option_depth3 -->
							<span class="fr_bullet">3차</span>
						</td>
						<td colspan="3">

							<?php
								// ------------ 3차 옵션 loop ------------
								$que3 = " select * from smart_common_option where co_suid='".$pass_common_uid."' and co_depth='3' and co_parent='$r[co_uid],$r2[co_uid]' order by co_sort asc , co_uid asc ";
								$res3 = _MQ_assoc($que3);
								if(sizeof($res3) <= 0){
									...
									<tbody>
										<?php
											foreach($res3 as $k3=>$r3) {
												if($r3['co_poptionname'] == "" || !$r3['co_poptionname'])  $save_chk ++;
										?>
											<tr class="depth_2<?php echo ($r3['co_view'] == 'Y'?null:' if_option_hide'); ?>">
												<td class="option_depth1 option_name">
													<input type="text" name="co_info[<?php echo $r3['co_uid']; ?>][co_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo $r3['co_poptionname']; ?>" style="width:<?=($app_option3_type == 'color' ? '200px;' : '100%')?>" />
													<?php
														// ----- 컬러형일 경우 -----
														if($app_option3_type == 'color') { fn_common_option_color( $r3['co_uid'] , $r3['co_color_type'] , $r3['co_color_name']); }
													?>
												</td>
												<td><input type="text" name="co_info[<?php echo $r3['co_uid']; ?>][co_poption_supplyprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['co_poption_supplyprice']); ?>" style="width:100%" /></td>
												<td><input type="text" name="co_info[<?php echo $r3['co_uid']; ?>][co_poptionprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['co_poptionprice']); ?>" style="width:100%" /></td>
												<td><input type="text" name="co_info[<?php echo $r3['co_uid']; ?>][co_cnt]" class="design number_style" placeholder="" value="<?php echo number_format($r3['co_cnt']); ?>" style="width:100%" /></td>
												<td>
													<div class="lineup-center">
														<?php echo _InputRadio('co_info['. $r3['co_uid'] .'][co_view]', array('Y', 'N'), ($r3['co_view']?$r3['co_view']:'Y'), ' class="btn_hide_input" ', array('노출', '숨김')); ?>
													</div>
												</td>
												<td>
													<div class="lineup-center">
														<a href="#none" onclick="f_sort('U' , '3', '<?php echo $r3['co_uid']; ?>' ); return false;" class="c_btn h22 icon_up" title="위로"></a>
														<a href="#none" onclick="f_sort('D' , '3', '<?php echo $r3['co_uid']; ?>' ); return false;" class="c_btn h22 icon_down" title="아래로"></a>
														<a href="#none" onclick="f_insert('3', '<?php echo $r3['co_uid']; ?>'); return false;" class="c_btn h22">삽입</a><!-- 끼워넣기 추가버튼 -->
														<a href="#none" onclick="category_apply_save('3depth_del', '<?php echo $r3['co_uid']; ?>'); return false;" class="c_btn h22 gray">삭제</a>
													</div>
												</td>
											</tr>
										<?php
