<?php
	...
	for($i=1 ; $i<=date("t" , strtotime(date("{$pass_sdate}-01"))) ; $i++){
		...
		// 미래 데이터가 보이지 않게 함.
		if($app_key > date("Y-m-d")){ continue; }
...
?>
					...
					<table class="table_form">
						...
						<tbody>
							<tr>
								<th>기간선택</th>
								<td>
									<input type="text" name="pass_sdate" class="design js_pic_month_max_today" value="<?php echo $pass_sdate; ?>" style="width:70px">
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="group_title"><strong><?=date("Y" , strtotime($pass_sdate))?>년 <?=date("m" , strtotime($pass_sdate))?>월 방문자분석 요약</strong></div>
			<div class="data_list">
				<table class="table_list">
					...
					<thead>
						<tr>
							<th rowspan="2" scope="col">총 방문수</th>
							<th colspan="2" scope="col">접속기기구분</th>
							<th colspan="2" scope="col">회원구분</th>
						</tr>
						<tr>
							<th scope="col">PC</th>
							<th scope="col">모바일</th>
				...
				<table class="table_list" style="margin-top:50px;">
					...
					<thead>
						<tr>
							<th scope="col" >접속기기별 방문수 그래프</th>
							<th scope="col" >회원구분별 방문수 그래프</th>
						</tr>
					</thead> 
					<tbody> 
						<tr>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('cntlog_day_search');
...
<?php
	...
	foreach( $res_date as $datek => $datev ){
		...
		$arr_table_data[] = array(
				...
				'rdate' => $datev['rdate'] .' <span class="t_orange bold">('. DateToWeekName($datev['rdate']) .')' ,
