...
?>
				...
				<!-- 리스트영역 -->
				<div class="group_title"><strong><?=date("Y년 m월 d일" , strtotime($pass_sdate))?> ~ <?=date("Y년 m월 d일" , strtotime($pass_edate))?> 방문자분석 요약</strong></div>
				<div class="data_list">
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('cntlog_week_search');
...
<?php
	...
	foreach( $res_date as $datek => $datev ){
		...
		$arr_table_data[] = array(
				...
				'rdate' => week_name($datev['rdate'] , '요일') ,
