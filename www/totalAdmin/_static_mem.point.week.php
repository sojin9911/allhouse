...
for($i=0 ; $i<=6 ; $i++){
	...
	$arr_date_num['pay_ing'][$i] = $arr_data['pay_ing'][$i] * 1;// 선택시간 지급예정액
	$arr_date_num['pay_end'][$i] = $arr_data['pay_end'][$i] * 1;// 선택시간 지급액
	$arr_date_num['use_end'][$i] = $arr_data['use_end'][$i] * 1;// 선택시간 사용액
	...
	// ------------------------ 기간내 가입 회원 수 평균 ------------------------
	$arr_tot_date_num[$i] = $arr_tot_data[$i] * 1;// 선택시간
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 적립금 분석 Summary ------- ?>
				<div class="group_title"><strong><?=DATE("Y년 m월 d일" , strtotime($pass_date) )?> ~ <?=DATE("Y년 m월 d일" , strtotime($pass_edate) )?> 요일별 회원 적립금 분석 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('point_week_search');
...
<?php
	...
	foreach( $arr_tot_cumul as $mem_datek => $mem_datev ){
		...
		$app_date = week_name( $mem_datek , '요일');
