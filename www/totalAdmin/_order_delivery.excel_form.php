<?php
$app_current_link = '_order_delivery.list.php';
...
// 엑셀 간략 검증
$OCheck = _MQ(" select o_ordernum from smart_order where o_ordernum = '{$Excel[2][0]}' ");
if(!$OCheck['o_ordernum']) error_msg('엑셀 파일이 잘못되었습니다.\\n\\n배송주문관리에서 받은 엑셀 파일이 맞는지 확인바랍니다.');
?>
<form action="_order_delivery.pro.php" method="post" onsubmit="return wFun();">
	<input type="hidden" name="_mode" value="ins_excel">
	<div class="group_title">
		<strong>배송주문관리 일괄업로드</strong>
			...
			<tbody>
				...
				?>
					<tr>
						...
						<td>
							<?php echo _InputSelect('w_check['.$_ordernum.']', array('Y', 'N'), 'Y', '', array('등록', '제외'), ''); ?>
						</td>
						<td>
							<?php
							$DClass = ''; // 배송상태 셀렉트박스 클래스
							if($v[13] == '배송완료') $DClass = 'diliver_ok';
							else if($v[13] == '배송중') $DClass = 'diliver_ing';
							else if($v[13] == '배송준비') $DClass = 'diliver_ready';
							else if($v[13] == '구매발주') $DClass = 'pay_ready';
							else $DClass = 'diliver_ready';
							echo _InputSelect('_sendstatus['.$_ordernum.']', $arr_order_product_sendstatus, ($v[13]?$v[13]:'배송중'), ' class="js_sendstatus '.($DClass?$DClass:null).'"', '', '');
							?>
						</td>
						<td>
							<?php echo _InputSelect('_sendcompany['.$_ordernum.']', array_keys($arr_delivery_company), $v[14], '', '', ''); ?>
						</td>
						<td>
							<input type="text" name="_sendnum[<?php echo $_ordernum; ?>]" class="design" placeholder="송장번호" value="<?php echo $v[15]; ?>">
						</td>
						<td><?php echo $_ordernum; ?></td>
						<td><?php echo $v[1]; ?></td>
						<td><?php echo $v[2]; ?></td>
						<td><?php echo $v[3]; ?></td>
						<td><?php echo $v[4]; ?></td>
						...
						<td><?php echo $v[7]; ?></td>
						<td><?php echo $v[8]; ?></td>
						<td class="t_left"><?php echo $v[9]; ?></td>
						<td class="t_left"><?php echo $v[10]; ?></td>
						<td class="t_left"><?php echo $v[11]; ?></td>
						<td><?php echo number_format((int)$v[12]); ?></td>
						<td><?php echo htmlspecialchars($v[16]); ?></td>
						<td><?php echo htmlspecialchars($v[17]); ?></td>
					</tr>
				<?php } ?>
			</tbody>
		</table>
