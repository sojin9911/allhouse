...
<div class="popup" style="border:0;">
	...
	<div class="pop_title"><strong>메인상품 등록/수정</strong></div>
	...
	<?php
		...
		// 추가파라메터
		$arr_param = array('pass_uid'=>$pass_uid);
		...
		// -- 기존에 선택된건 나오지 않게한다. 
		$s_query .= " and (select count(*) as cnt from smart_display_main_product where dmp_pcode = p.p_code and dmp_dmsuid = '".$pass_uid."' ) < 1  ";
	...
	<form name="mainProductForm">
	<input type="hidden" name="_uid" value=<?php echo $pass_uid; ?>>
				...
				<tbody>
					...
					?>
							<tr>
								<td>
									<label class="design">
										<input type="checkbox" name="chk_pcode[]" class="js_ck chk_box main-pcode" value="<?php echo $v['p_code']; ?>">
									</label>
								</td>
								<td><?php echo $_num; ?></td>
		...
		<div class="c_btnbox">
			<ul>
				<li><span class="c_btn h34 black"><input type="button" name="" value="선택적용" onclick="mainProductAdd();"  /></span></li>
...
<script>
	// -- 메인상품 선택적용 클릭할 시
	function mainProductAdd()
	{
		var _uid = $('[name="mainProductForm"] [name="_uid"]').val();
			var chkLen = $('.main-pcode:checked').length;
			if( chkLen < 1){ alert("한개 이상 선택해 주세요."); return false; }
			var selectVar = $('.main-pcode:checked').serialize();
			var url = '_config.display.main.ajax_pro.php';
	    $.ajax({
	        url: url, cache: false,dataType : 'json', type: "POST", 
	        data: {_mode : 'selectMainProductAdd' , selectVar : selectVar , _uid : _uid }, 
	        success: function(data){
	        	if(data.rst == 'success'){
	        		opener.selectMainProductList(); // 다시 로드
	        		window.close();
	        	}else{
	        		alert(data.msg);
	        		return false;
