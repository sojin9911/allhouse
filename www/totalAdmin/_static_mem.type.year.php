...
// ------- 전체 회원수 -------
$mem_tot_sum = _MQ("SELECT count(*) as total_plus FROM smart_individual WHERE IFNULL(in_rdate,0) > 0 ");
...
$res = _MQ_assoc(" SELECT LEFT(in_rdate,4) as rdate ,count(*) as total_plus FROM smart_individual GROUP BY rdate ORDER BY rdate ASC ");
foreach($res as $k=>$v){
	if($v['rdate'] > 0 ) {
...
for($i=$year_min ; $i<=$year_max ; $i++){
	...
	// ------------------------ 기간 내 상태 분석 ------------------------
	$arr_date_num['auth'][$i] = $arr_data['auth'][sprintf("%04d" , $i)] * 1;// 선택월 승인 회원수
	$arr_date_num['sleep'][$i] = $arr_data['sleep'][sprintf("%04d" , $i)] * 1;// 선택월 휴면 회원수
	$arr_date_num['out'][$i] = $arr_data['out'][sprintf("%04d" , $i)] * 1;// 선택월 탈퇴 회원수
	...
	// 승인 - 최대값 체크 
	$arr_maxmin['auth']['max'] = ( $arr_maxmin['auth']['max']['cnt'] < $arr_date_num['auth'][$i] ? array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%04d" , $i)) : $arr_maxmin['auth']['max']);

	// 휴면 - 최대값 체크 
	$arr_maxmin['sleep']['max'] = ( $arr_maxmin['sleep']['max']['cnt'] < $arr_date_num['sleep'][$i] ? array('cnt'=>$arr_date_num['sleep'][$i] , 'date'=>sprintf("%04d" , $i)) : $arr_maxmin['sleep']['max']);


	// 승인 - 등록되어 있지 않은 경우 무조건 등록
	if(!isset($arr_maxmin['auth']['min']['cnt'])) {
		$arr_maxmin['auth']['min'] = array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%04d" , $i));
		$arr_avg['auth']['idx'] ++; // 계산할 횟수
		$arr_avg['auth']['cnt'] += $arr_date_num['auth'][$i]; // 계산할 수
	}
	// 승인 - 최소 정보일 경우 현시간 제외
	else if(date("Y") > sprintf("%04d" , $i)){
		$arr_maxmin['auth']['min'] = ( $arr_maxmin['auth']['min']['cnt'] > $arr_date_num['auth'][$i] ? array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%04d" , $i)) : $arr_maxmin['auth']['min']);
		$arr_avg['auth']['idx'] ++; // 계산할 횟수
		$arr_avg['auth']['cnt'] += $arr_date_num['auth'][$i]; // 계산할 수
	}
	...
	if(!isset($arr_maxmin['sleep']['min']['cnt'])) {
		$arr_maxmin['sleep']['min'] = array('cnt'=>$arr_date_num['sleep'][$i] , 'date'=>sprintf("%04d" , $i));
		$arr_avg['sleep']['idx'] ++; // 계산할 횟수
		$arr_avg['sleep']['cnt'] += $arr_date_num['sleep'][$i]; // 계산할 수
	}
	// 휴면 - 최소 정보일 경우 현시간 제외
	else if(date("Y") > sprintf("%04d" , $i)){
		$arr_maxmin['sleep']['min'] = ( $arr_maxmin['sleep']['min']['cnt'] > $arr_date_num['sleep'][$i] ? array('cnt'=>$arr_date_num['sleep'][$i] , 'date'=>sprintf("%04d" , $i)) : $arr_maxmin['sleep']['min']);
		$arr_avg['sleep']['idx'] ++; // 계산할 횟수
		$arr_avg['sleep']['cnt'] += $arr_date_num['sleep'][$i]; // 계산할 수
	}
	...
	if(!isset($arr_maxmin['out']['min']['cnt'])) {
		$arr_maxmin['out']['min'] = array('cnt'=>$arr_date_num['out'][$i] , 'date'=>sprintf("%04d" , $i));
		$arr_avg['out']['idx'] ++; // 계산할 횟수
		$arr_avg['out']['cnt'] += $arr_date_num['out'][$i]; // 계산할 수
	}
	// 탈퇴 - 최소 정보일 경우 현시간 제외
	else if(date("Y") > sprintf("%04d" , $i)){
		$arr_maxmin['out']['min'] = ( $arr_maxmin['out']['min']['cnt'] > $arr_date_num['out'][$i] ? array('cnt'=>$arr_date_num['out'][$i] , 'date'=>sprintf("%04d" , $i)) : $arr_maxmin['out']['min']);
		$arr_avg['out']['idx'] ++; // 계산할 횟수
		$arr_avg['out']['cnt'] += $arr_date_num['out'][$i]; // 계산할 수
	}
	// ------------------------ 기간 내 상태 분석 ------------------------
	...
	// ------------------------ 기간내 가입 회원 수 ------------------------
	$arr_tot_date_num[$i] = $arr_tot_data[sprintf("%04d" , $i)] * 1;// 선택일자 회원수
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 상태 분석 Summary ------- ?>
				<div class="group_title"><strong>전체 회원 상태 분석 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('type_year_search');
