...
if($pass_sdate && $pass_edate)  $s_query .= " AND (date(o_rdate) between '{$pass_sdate}' and '{$pass_edate}') ";// - 검색기간
else if($pass_sdate) $s_query .= " AND date(o_rdate) >= '{$pass_sdate}' ";
else if($pass_edate) $s_query .= " AND date(o_rdate) <= '{$pass_edate}' ";;
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td<?php echo $TDStyle; ?>><?php echo $sv['cp_id_name']; ?></td>
