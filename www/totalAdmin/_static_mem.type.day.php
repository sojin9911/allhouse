...
$res = _MQ_assoc(" SELECT DATE(in_rdate) as rdate ,count(*) as total_plus FROM smart_individual WHERE LEFT(in_rdate,7) = '". $pass_date ."' GROUP BY rdate ORDER BY rdate ASC ");
foreach($res as $k=>$v){
	$arr_tot_cumul[DATE("d" , strtotime($v['rdate']))] = array(
		'idx' => $arr_tot_cumul[DATE("d" , strtotime($v['rdate']))]['idx'] + 1 , // 횟수
		'cnt' => $arr_tot_cumul[DATE("d" , strtotime($v['rdate']))]['cnt'] + $v['total_plus'] // 누적수
	);
}
...
for($i=1 ; $i<=date("t" , strtotime(date("{$Select_Year}-{$Select_Month}-01"))) ; $i++){
	...
	$arr_date_date[$i] = $i . "일";// 일자

	// ------------------------ 기간 내 상태 분석 ------------------------
	$arr_date_num['auth'][$i] = $arr_data['auth'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 승인 회원수
	$arr_date_num['sleep'][$i] = $arr_data['sleep'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 휴면 회원수
	$arr_date_num['out'][$i] = $arr_data['out'][$Select_Year . $Select_Month . sprintf("%02d" , $i)] * 1;// 선택일자 탈퇴 회원수
	...
	// 승인 - 최대값 체크 
	$arr_maxmin['auth']['max'] = ( $arr_maxmin['auth']['max']['cnt'] < $arr_date_num['auth'][$i] ? array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['auth']['max']);
	...
	if(!isset($arr_maxmin['auth']['min']['cnt'])) {
		$arr_maxmin['auth']['min'] = array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%02d" , $i));
		$arr_avg['auth']['idx'] ++; // 계산할 횟수
		$arr_avg['auth']['cnt'] += $arr_date_num['auth'][$i]; // 계산할 수
	}
	// 승인 - 최소 정보일 경우 현시간 제외
	else if(date("Ymd") > $Select_Year . $Select_Month . sprintf("%02d" , $i)){
		$arr_maxmin['auth']['min'] = ( $arr_maxmin['auth']['min']['cnt'] > $arr_date_num['auth'][$i] ? array('cnt'=>$arr_date_num['auth'][$i] , 'date'=>sprintf("%02d" , $i)) : $arr_maxmin['auth']['min']);
		$arr_avg['auth']['idx'] ++; // 계산할 횟수
		$arr_avg['auth']['cnt'] += $arr_date_num['auth'][$i]; // 계산할 수
	}
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 상태 분석 Summary ------- ?>
				<div class="group_title"><strong><?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 회원 상태 분석 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('type_day_search');
	...
	var chartData = {
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 승인 회원수',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 휴면 회원수',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 탈퇴 회원수',
		...
		{
			type: 'line',
			label: '<?php echo $Select_Year; ?>년 <?php echo $Select_Month; ?>월 가입 회원수',
