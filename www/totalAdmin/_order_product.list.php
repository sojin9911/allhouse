...
// 검색 조건
$s_query = " and o.o_canceled = 'N' and o.o_paystatus = 'Y' and npay_order = 'N' and op.op_cancel = 'N' "; // 기본조건(취소 되지 않고 결제 상태인것) / 네이버페이 제외
		...
		<table class="table_form">
			...
			<tbody>
				...
				<tr>
					<td colspan="6">
						<div class="tip_box">
						<?php echo _DescStr('네이버페이 주문정보는 별도 관리되며 정산처리가 불가능합니다. 네이버주문은 네이버페이 센터를 이용바랍니다.'); ?>
	...
	<form action="_order_product.pro.php" method="post" class="form_list"<?php echo ($c?null:' target="common_frame"'); ?>>
		...
		<div class="list_ctrl">
			<div class="left_box">
				...
				<a href="#none" onclick="settlement_status('ready'); return false;" class="c_btn h27 gray">선택 정산대기</a>
						...
						<tr data-uid="<?php echo $v['op_uid']; ?>">
							...
							<td>
								<div class="lineup-center">
									...
									if($v['op_sendstatus'] == '구매발주') $DClass = 'pay_ready';
							...
							<td>
								<?php
								if($v['op_sendstatus'] != '구매발주' && $v['op_sendcompany']) {
									$DLink = '';
									$DLinkJS = null;
							...
							<td>
								<div class="lineup-vertical">
									<a href="#none" onclick="window.open('<?php echo OD_PROGRAM_URL; ?>/mypage.order.mass.print_view.php<?php echo URI_Rebuild('?', array('_mode'=>'print', 'ordernum'=>$v['op_oordernum'], '_PVSC'=>$_PVSC)); ?>' ,'print','width=860,height=820,scrollbars=yes'); return false;" class="c_btn h22">주문인쇄</a>
