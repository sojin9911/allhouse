...
$Select_Year = $pass_date;
	...
	foreach( $res as $k=>$v ){
		...
		$arr_data['mobileY'][str_replace("-" , "" , $v['rdate'])] = $v['mobileY_sum_price_real'];
		$app_max['mobileY'] = ($app_max['mobileY'] < $v['mobileY_sum_price_real'] ? $v['mobileY_sum_price_real'] : $app_max['mobileY']);

		$arr_data['mobileN'][str_replace("-" , "" , $v['rdate'])] = $v['mobileN_sum_price_real'];
		...
		$arr_tot_cumul[end(explode("-" , $v['rdate']))] = array(
			...
			'cnt' => $arr_tot_cumul[end(explode("-" , $v['rdate']))]['cnt'] + $v['mobileY_sum_price_real'] + $v['mobileN_sum_price_real'] // 누적수
		);
	...
	for($i=1 ; $i<=12 ; $i++){
		...
		$arr_date_num['mobileN'][$i] = $arr_data['mobileN'][$Select_Year . sprintf("%02d" , $i)] * 1;// 선택월 PC 매출
		$arr_date_num['mobileY'][$i] = $arr_data['mobileY'][$Select_Year . sprintf("%02d" , $i)] * 1;// 선택월 모바일 매출
		...
		// PC - 최소 정보일 경우 현시간 제외
		else if(date("Ymd") > $Select_Year . sprintf("%02d" , $i)){
	...
	<tbody>
		<tr>
			<td >
				...
				<? // ------- 매출 Summary ------- ?>
				<div class="group_title"><strong><?php echo $Select_Year; ?>년 매출 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('all_month_search');
...
<script>
	...
	var chartData = {
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 PC 매출',
		...
		{
			type: 'bar',
			label: '<?php echo $Select_Year; ?>년 모바일 매출',
		...
		{
			type: 'line',
			label: '<?php echo $Select_Year; ?>년 매출',
