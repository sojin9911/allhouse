...
<input type="hidden" name="pass_uid" value="">
			...
			<table class="table_list">
				...
				<tbody>
					...
					?>
						<tr class="depth_2<?php echo ($r['po_view'] == 'Y'?null:' if_option_hide'); ?>">
							<td class="option_depth1 option_name">
								<?php // KAY :: 일괄업로드 :: 2021-07-02 -- 옵션명에서 특수문자 제거 ?>
								<?php $op_str = array(">","|","§"); //변경할 특수문자 종류 배열?>
								<input type="text" name="po_info[<?php echo $r['po_uid']; ?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r['po_poptionname']); ?>" style="width:<?=($app_option1_type == 'color' ? '200px;' : '100%')?>" />
								<?php
									// ----- 컬러형일 경우 -----
									if($app_option1_type == 'color') { fn_option_color( $r['po_uid'] , $r['po_color_type'] , $r['po_color_name']); }
								?>
							</td>
							<td><input type="text" name="po_info[<?php echo $r['po_uid']; ?>][po_poption_supplyprice]" class="design number_style" placeholder="" value="<?php echo number_format($r['po_poption_supplyprice']); ?>" style="width:100%" /></td>
							<td><input type="text" name="po_info[<?php echo $r['po_uid']; ?>][po_poptionprice]" class="design number_style" placeholder="" value="<?php echo number_format($r['po_poptionprice']); ?>" style="width:100%" /></td>
							<td><input type="text" name="po_info[<?php echo $r['po_uid']; ?>][po_cnt]" class="design number_style" placeholder="" value="<?php echo number_format($r['po_cnt']); ?>" style="width:100%" /></td>
							<td><input type="text" name="" class="design" placeholder="" value="<?php echo number_format($r['po_salecnt']); ?>" style="width:100%" disabled /></td>
							<td>
							...
							<td>
								<div class="lineup-center">
									...
									<a href="#none" onclick="f_insert('1', '<?php echo $r['po_uid']; ?>'); return false;" class="c_btn h22">삽입</a><!-- 끼워넣기 추가버튼 -->
