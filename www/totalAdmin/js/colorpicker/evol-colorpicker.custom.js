...
$.widget( "evol.colorpicker", {
	...
	_paletteHTML: function() {
		...
		var h='<div class="evo-pop'+_ie+' ui-widget ui-widget-content ui-corner-all"'+
			(this._isPopup?' style="position:absolute;margin-left:'+(this.options.hideButton?this.element.width():this.element.width()+25)+'px;margin-top:30px;"':' style="margin-left:'+(this.options.hideButton?this.element.width():this.element.width()+25)+'px;margin-top:30px;"')+'>'+
			// palette
	...
	_downOrUpPositioning: function() {
		...
		while (el !== null && i < 100) {
			...
			if (el.css('overflow') != 'visible') {
				...
				// JJC : 무조건 위로 적용 -- 이를 위해 evol-colorpicker.min.css 파일 맨 앞에 .evo-cp-wrap{	position: relative;} 넣기  : 2018-04-18
				this._palette.css({ bottom: '3px' });
				//if (openUp) {
