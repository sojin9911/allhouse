...
<div class="popup" style="border:0;">
	...
	<?php
		...
		// -- 기존에 선택된건 나오지 않게한다. 
		$s_query .= " and (select count(*) as cnt from smart_display_type_product where dtp_pcode = p.p_code and dtp_dtsuid = '".$pass_uid."' ) < 1  ";
	...
	<input type="hidden" name="_uid" value=<?php echo $pass_uid; ?>>
				...
				<tbody>
					...
					?>
							<tr>
								<td>
									<label class="design">
										<input type="checkbox" name="chk_pcode[]" class="js_ck chk_box type-pcode" value="<?php echo $v['p_code']; ?>">
									</label>
								</td>
								<td><?php echo $_num; ?></td>
		...
		<div class="c_btnbox">
			<ul>
				<li><span class="c_btn h34 black"><input type="button" name="" value="선택적용" onclick="typeProductAdd();"  /></span></li>
...
<script>
	// -- 메인상품 선택적용 클릭할 시
	function typeProductAdd()
	{
		var _uid = $('[name="typeProductForm"] [name="_uid"]').val();
			var chkLen = $('.type-pcode:checked').length;
			if( chkLen < 1){ alert("한개 이상 선택해 주세요."); return false; }
			var selectVar = $('.type-pcode:checked').serialize();
			var url = '_config.display.type.ajax_pro.php';
	    $.ajax({
	        url: url, cache: false,dataType : 'json', type: "POST", 
	        data: {_mode : 'selectTypeProductAdd' , selectVar : selectVar , _uid : _uid }, 
	        success: function(data){
	        	if(data.rst == 'success'){
	        		opener.selectTypeProductList(); // 다시 로드
	        		window.close();
	        	}else{
	        		alert(data.msg);
	        		return false;
