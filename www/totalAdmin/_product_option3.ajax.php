...
switch( $pass_mode ){
    ...
    case "1depth_del":
        ...
        if(in_array($pass_mode , array("1depth_del" , "2depth_del"))) {
            // 삭제전 하위정보 확인
            $ique = " select count(*) as cnt from smart_product_option where po_pcode='{$pass_code}' and find_in_set('{$pass_uid}' , po_parent) > 0 and po_depth <= '". (rm_str($pass_mode)+1) ."' "; // 1차 정보
            $ires = _MQ($ique);
            if($ires['cnt'] > 0) {
    ...
    case "3depth_add":
        ...
        $ique = " select * from smart_product_option where po_pcode='{$pass_code}' and po_uid='{$pass_uid}' "; // 2차 정보
        $ir = _MQ($ique);

        // 순번추출
        $r = _MQ(" select ifnull(max(po_sort),0) as max_sort from smart_product_option where po_pcode='" . $pass_code . "' and po_depth='3' and po_parent='" . $ir['po_parent'].",".$ir['po_uid'] . "' ");
        ...
        _MQ_noreturn("
            insert smart_product_option set
                ...
                po_poptionname='',
                po_depth='3',
                po_parent='{$ir['po_parent']},{$ir['po_uid']}',
                po_sort='". $max_sort ."'
        ");
    ...
    case "2depth_add":
        ...
        _MQ_noreturn("
            insert smart_product_option set
                ...
                po_poptionname='',
                po_depth='3',
                po_parent='{$ir['po_uid']},{$uid_2depth}',
                po_sort='". $max_sort3 ."'
        ");
			...
			<tbody>
				...
				?>
					...
					<tr>
						...
						<td>
							<?php //KAY : 상품일괄업로드 개선 : 2021-07-02 -- 옵션명에서 특수문자 제거 ?>
							<input type="text" name="po_info[<?php echo $r2['po_uid']; ?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r2['po_poptionname']); ?>"   style="width:<?=($app_option2_type == 'color' ? '300px;' : '100%')?>" />
							<?php
								// ----- 컬러형일 경우 -----
						...
						<td>
							<div class="lineup-center">
								...
								<a href="#none" onclick="category_apply_save('3depth_add', '<?php echo $r2['po_uid']; ?>'); return false;" class="c_btn h22 ">추가</a><!-- 끼워넣기 추가버튼 -->
					...
					<tr>
						...
						<td colspan="3">
							...
							<?php
								// ------------ 3차 옵션 loop ------------
								$que3 = " select * from smart_product_option where po_pcode='{$pass_code}' and po_depth='3' and po_parent='$r[po_uid],$r2[po_uid]' order by po_sort asc , po_uid asc ";
								$res3 = _MQ_assoc($que3);
								if(sizeof($res3) <= 0){
									...
									<tbody>
										<?php
											foreach($res3 as $k3=>$r3) {
												if($r3['po_poptionname'] == "" || !$r3['po_poptionname'])  $save_chk ++;
										?>
											<tr class="depth_2<?php echo ($r3['po_view'] == 'Y'?null:' if_option_hide'); ?>">
												<td class="option_depth1 option_name">
													<?php //KAY : 상품일괄업로드 개선 : 2021-07-02 -- 옵션명에서 특수문자 제거 ?>
													<input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poptionname]" class="design" placeholder="옵션명을 입력해주세요." value="<?php echo str_replace($op_str,"",$r3['po_poptionname']); ?>"   style="width:<?=($app_option3_type == 'color' ? '200px;' : '100%')?>" />
													<?php
														// ----- 컬러형일 경우 -----
														if($app_option3_type == 'color') { fn_option_color( $r3['po_uid'] , $r3['po_color_type'] , $r3['po_color_name']); }
													?>
												</td>
												<td><input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poption_supplyprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_poption_supplyprice']); ?>" style="width:100%" /></td>
												<td><input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_poptionprice]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_poptionprice']); ?>" style="width:100%" /></td>
												<td><input type="text" name="po_info[<?php echo $r3['po_uid']; ?>][po_cnt]" class="design number_style" placeholder="" value="<?php echo number_format($r3['po_cnt']); ?>" style="width:100%" /></td>
												<td><input type="text" name="" class="design" placeholder="" value="<?php echo number_format($r3['po_salecnt']); ?>" style="width:100%" disabled /></td>
												<td>
													<div class="lineup-center">
														<?php echo _InputRadio('po_info['. $r3['po_uid'] .'][po_view]', array('Y', 'N'), ($r3['po_view']?$r3['po_view']:'Y'), ' class="btn_hide_input" ', array('노출', '숨김')); ?>
													</div>
												</td>
												<td>
													<div class="lineup-center">
														<a href="#none" onclick="f_sort('U' , '3', '<?php echo $r3['po_uid']; ?>' ); return false;" class="c_btn h22 icon_up" title="위로"></a>
														<a href="#none" onclick="f_sort('D' , '3', '<?php echo $r3['po_uid']; ?>' ); return false;" class="c_btn h22 icon_down" title="아래로"></a>
														<a href="#none" onclick="f_insert('3', '<?php echo $r3['po_uid']; ?>'); return false;" class="c_btn h22">삽입</a><!-- 끼워넣기 추가버튼 -->
														<a href="#none" onclick="category_apply_save('3depth_del', '<?php echo $r3['po_uid']; ?>'); return false;" class="c_btn h22 gray">삭제</a>
													</div>
												</td>
											</tr>
										<?php
