...
for($i=0 ; $i<=23 ; $i++){
	...
	$arr_date_num['in_join_ua_PC_plus'][$i] = $arr_data['in_join_ua_PC_plus'][$i] * 1;// 선택일자 PC 회원수
	$arr_date_num['in_join_ua_MOBILE_plus'][$i] = $arr_data['in_join_ua_MOBILE_plus'][$i] * 1;// 선택일자 MOBILE 회원수
	...
	// PC - 최대값 체크 
	$arr_maxmin['in_join_ua_PC_plus']['max'] = ( $arr_maxmin['in_join_ua_PC_plus']['max']['cnt'] < $arr_date_num['in_join_ua_PC_plus'][$i] ? array('cnt'=>$arr_date_num['in_join_ua_PC_plus'][$i] , 'date'=>$i) : $arr_maxmin['in_join_ua_PC_plus']['max']);

	// MOBILE - 최대값 체크 
	$arr_maxmin['in_join_ua_MOBILE_plus']['max'] = ( $arr_maxmin['in_join_ua_MOBILE_plus']['max']['cnt'] < $arr_date_num['in_join_ua_MOBILE_plus'][$i] ? array('cnt'=>$arr_date_num['in_join_ua_MOBILE_plus'][$i] , 'date'=>$i) : $arr_maxmin['in_join_ua_MOBILE_plus']['max']);


	// PC - 등록되어 있지 않은 경우 무조건 등록
	if(!isset($arr_maxmin['in_join_ua_PC_plus']['min']['cnt'])) {
		$arr_maxmin['in_join_ua_PC_plus']['min'] = array('cnt'=>$arr_date_num['in_join_ua_PC_plus'][$i] , 'date'=>$i);
		$arr_avg['in_join_ua_PC_plus']['idx'] ++; // 계산할 횟수
		$arr_avg['in_join_ua_PC_plus']['cnt'] += $arr_date_num['in_join_ua_PC_plus'][$i]; // 계산할 접속수
	}
	// PC - 최소 정보일 경우 현시간 제외
	else {
		$arr_maxmin['in_join_ua_PC_plus']['min'] = ( $arr_maxmin['in_join_ua_PC_plus']['min']['cnt'] > $arr_date_num['in_join_ua_PC_plus'][$i] ? array('cnt'=>$arr_date_num['in_join_ua_PC_plus'][$i] , 'date'=>$i) : $arr_maxmin['in_join_ua_PC_plus']['min']);
		$arr_avg['in_join_ua_PC_plus']['idx'] ++; // 계산할 횟수
		$arr_avg['in_join_ua_PC_plus']['cnt'] += $arr_date_num['in_join_ua_PC_plus'][$i]; // 계산할 접속수
	}
	...
	if(!isset($arr_maxmin['in_join_ua_MOBILE_plus']['min']['cnt'])) {
		$arr_maxmin['in_join_ua_MOBILE_plus']['min'] = array('cnt'=>$arr_date_num['in_join_ua_MOBILE_plus'][$i] , 'date'=>$i);
	...
	// MOBILE - 최소 정보일 경우 현시간 제외
		...
		$arr_maxmin['in_join_ua_MOBILE_plus']['min'] = ( $arr_maxmin['in_join_ua_MOBILE_plus']['min']['cnt'] > $arr_date_num['in_join_ua_MOBILE_plus'][$i] ? array('cnt'=>$arr_date_num['in_join_ua_MOBILE_plus'][$i] , 'date'=>$i) : $arr_maxmin['in_join_ua_MOBILE_plus']['min']);
		$arr_avg['in_join_ua_MOBILE_plus']['idx'] ++; // 계산할 횟수
		$arr_avg['in_join_ua_MOBILE_plus']['cnt'] += $arr_date_num['in_join_ua_MOBILE_plus'][$i]; // 계산할 접속수
	}
	// ------------------------ 가입기기별 ------------------------
	...
	<tbody>
		...
		<tr>
			<td colspan="2" >
				...
				<? // ------- 회원 가입형태 분석 Summary ------- ?>
				<div class="group_title"><strong><?=DATE("Y년 m월 d일" , strtotime($pass_date) )?> ~ <?=DATE("Y년 m월 d일" , strtotime($pass_edate) )?> 시간별 회원 가입형태 분석 요약</strong></div>
...
<script>
	...
	function searchExcel() {
		$('form[name="frmSearch"]').children('input[name="_mode"]').val('method_hour_search');
