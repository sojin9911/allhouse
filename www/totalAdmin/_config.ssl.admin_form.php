...
<input type='hidden' name='pass_menu' value='_config.ssl.admin_form'>



	<!-- 관리자 보안서버 설정 -->
	<div class="group_title"><strong>관리자 보안서버 설정</strong></div>
	<div class="data_form">
		<table class="table_form">
			...
			<tbody>
				<tr>
					...
					<td >
						<?=_InputRadio( "_ssl_admin_loc" , array("N" , "A" , "P") ,  (!$siteInfo['s_ssl_admin_loc'] ? "N" : $siteInfo['s_ssl_admin_loc'] ) , "" , array("미사용" , "전체 페이지" , "개인정보 이용 페이지") , "")?>
						<div class="tip_box">
							<?=_DescStr("개인정보 이용 페이지 : 회원관리, 주문관리, 게시판관리, 문의관리 등이며, 아래의 관리자 개인정보 이용페이지 섹션에 상세히 기술하였습니다.");?>
						</div>						
					</td>
				</tr>
					...
					<td >
						...
						<div style='clear:both; padding-top:5px;' ID="page_area">
							<?PHP
								...
								echo "<input type='hidden' name='page_add_cnt' value='" . ($siteInfo['s_ssl_admin_page'] ? sizeof($page_ex) : 1) . "'>";
								if(sizeof($page_ex) > 0 ) {
									echo "
						...
						<div class="tip_box">
							...
							<?=_DescStr("파일명만 입력하세요. 예) <strong>_individual.list.php</strong> ")?>
	...
	<!-- 관리자 보안서버 설정 -->
	<div class="group_title"><strong>관리자 개인정보 이용페이지</strong></div>
	<div class="data_form">
		<table class="table_form">
			<colgroup>
				<col width="180"/><col width="*"/>
			...
			<tbody>
				...
				<tr>
					<th>관리자 개인정보 이용페이지</th>
					<td >
						<?PHP
							if(sizeof($arr_ssl_admin_page) > 0 ) {
								echo "
									<!-- ● 데이터 리스트 --><table class='table_list'>
										...
										<tbody>
								";
								foreach( $arr_ssl_admin_page as $k=>$v ){
									echo '
										<tr>
...
<script>
	/*  ON/OFF ---------- */
	var onoff = function() {
		if($("input[name='_ssl_admin_loc']").filter(function() {if (this.checked) return this;}).val() == "P") {
			$(".auth_view").show();
		}
		else {
			$(".auth_view").hide();
	...
	onoff();
	$("input[name='_ssl_admin_loc']").click(function() {onoff();});
	/*  ON/OFF ---------- */
</script>
