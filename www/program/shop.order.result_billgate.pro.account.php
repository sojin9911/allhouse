...
include $_SERVER[DOCUMENT_ROOT]."/pg/pc/billgate/class/Message.php";
include $_SERVER[DOCUMENT_ROOT]."/pg/pc/billgate/class/MessageTag.php";
include $_SERVER[DOCUMENT_ROOT]."/pg/pc/billgate/class/ServiceCode.php";
include $_SERVER[DOCUMENT_ROOT]."/pg/pc/billgate/class/Command.php";
include $_SERVER[DOCUMENT_ROOT]."/pg/pc/billgate/class/ServiceBroker.php";
...
if(!strcmp($RESPONSE_CODE, "0000")) { // If authentication is successful, the payment request
	...
	if(!strcmp($checkSum, 'SUC')){
		//---------------------------------------
		// Request
		//---------------------------------------
		$broker->invokeMessage($svcCode->ACCOUNT_TRANSFER, $MESSAGE); //authentication request
		...
		if(!strcmp($resMsg->get($tag->RESPONSE_CODE), "0000")) {
			...
			$USING_TYPE 		= $resMsg->get($tag->USING_TYPE);
			$IDENTIFIER_TYPE= $resMsg->get($tag->IDENTIFIER_TYPE);
			$AUTH_DATEIDENTIFIER 		= $resMsg->get($tag->AUTH_DATEIDENTIFIER);
