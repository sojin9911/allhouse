<?PHP
	...
	foreach($_POST as $nmParam => $valParam)
	{
		...
		{
			$valParam = iconv('utf-8', 'euc-kr', $valParam);
	...
	if ( $req_tx == "cert" )
	{
		...
		// SSJ : KCP 본인인증 모듈 가맹점인증키 추가 패치 : 2021-03-12
		if($app_enc_key <> ''){
			$up_hash = $ct_cert->make_hash_data( $home_dir, $app_enc_key, $hash_data );
		}else{
		...
		// SSJ : KCP 본인인증 모듈 가맹점인증키 추가 패치 : 2021-03-12
        if($app_enc_key <> ''){ $sbParam .= "<input type='hidden' name='kcp_cert_lib_ver' value='" . $ct_cert->get_kcp_lib_ver( $home_dir ) . "'/>"; }
	}
        ...
        <script type="text/javascript">
            ...
            function cert_page()
            {
                ...
                else if ( frm.req_tx.value == "cert" )
                {
                    ...
                    {
                        window.opener.document.form_auth.veri_up_hash.value = frm.up_hash.value; // up_hash 데이터 검증을 위한 필드
                    }
                    frm.action="https://<?php echo ($siteInfo['s_join_auth_kcb_code'] == 'S6186' ? 'test' : null); ?>cert.kcp.co.kr/kcp_cert/cert_view.jsp";
                    frm.submit();
                }
            }
        </script>
