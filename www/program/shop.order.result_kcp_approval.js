	...
	function kcp_AJAX()
	{
		...
		var url    = "/program/shop.order.result_kcp_approval.php";
		...
		if ( req01_AJAX.readyState == READY_STATE_COMPLETE )
		{
			if ( req01_AJAX.status == 200 )
			{
				...
				if ( req01_AJAX.responseText != null )
				{
                    ...
                    //오류코드값분리    txt[0].replace(/^\s*/,'').replace(/\s*$/,'') 
                    var codedata = txt[0].replace(" ","");
                    var code = codedata.substring(codedata.length-4,codedata.length);
					if( code == '0000' )
					{
