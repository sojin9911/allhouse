...
# 데이터 조회
$s_query = '';
$s_query = " and lc_mid = '". get_userid() ."' ";
$listmaxcount = 20 ;
if( !$listpg ) {$listpg = 1 ;}
$count = $listpg * $listmaxcount - $listmaxcount;
$res = _MQ(" select count(*) as cnt from smart_loginchk where (1) {$s_query} ");
$TotalCount = $res['cnt'];
$Page = ceil($TotalCount / $listmaxcount);
$row = _MQ_assoc(" select * from smart_loginchk where (1) {$s_query} ORDER BY lc_rdate desc, lc_uid desc limit {$count}, {$listmaxcount} ");
