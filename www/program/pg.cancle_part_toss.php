...
if($tmp['cnt']==1) { $_total_amount = ''; } // 전체금액 취소

// 결제취소 API
$url = "https://api.tosspayments.com/v1/payments/{$paymentKey}/cancel";
$data = array('cancelReason' => '고객요청', 'cancelAmount' => $_total_amount); // cancelAmount가 없으면 전액 취소
...
if($rst['code'] == '' && in_array($rst['status'], array('CANCELED','PARTIAL_CANCELED'))){
	// 취소 성공 여부
	$is_pg_status = true;
}else{
	// 취소 성공 여부
