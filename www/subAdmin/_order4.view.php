...
if($r['s_partnerCode'] != $com_id) error_msg('잘못된 접근입니다.'); // 입점 관리자 로그인 아이디와 데이터의 입점 아이디가 틀린경우
...
if($arr_sum['total']['Y'] != 0 && $arr_sum['total']['N'] != 0) include_once(OD_ADMIN_ROOT.'/_order4.view.vat_double.php'); // 2가지 형태 추출 ( 과세 / 면세 )
else include_once(OD_ADMIN_ROOT.'/_order4.view.vat_one.php');// 한가지 형태 추출
