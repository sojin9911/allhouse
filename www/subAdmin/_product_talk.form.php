<?PHP
	...
	// - 수정 ---
	if( $_mode == "modify" ) {
		$que = " select *  from smart_product_talk as pt inner join smart_product as p on (pt.pt_pcode = p.p_code) where pt_uid='{$pt_uid}' and p.p_cpid = '{$com_id}' ";
		$row = _MQ($que);
...
<?PHP
	if( $pt_uid && $_mode == "add") {
		$ique = "
			...
			where pt.pt_uid='{$pt_uid}' and p.p_cpid = '{$com_id}'
		";
		$ir = _MQ($ique);
		if(!$ir['pt_uid']) error_msg('잘못된접근입니다.');
		...
		<table class="table_form">
			...
			<tbody>
				...
				<?php if($ir['pt_type'] == "상품평가" && $ir['pt_depth'] == 1) { ?>
					<tr>
						<th>평점</th>
						<td><?php echo $ir['pt_eval_point']; ?></td>
					</tr>
				<?php } ?>
			</tbody>
		</table>
