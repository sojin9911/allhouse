...
$app_current_link = '_product_talk.list.php?pt_type='.urlencode($_GET['pt_type']);
...
// 검색 체크
$s_query = "from smart_product_talk as pt left join smart_product as p on (pt.pt_pcode = p.p_code) where pt_depth = '1' and p.p_cpid = '{$com_id}' ";
				...
				?>
					<tr>
						...
						<td class="t_left">
							...
							<?php echo stripslashes(htmlspecialchars($v['p_name'])); ?>
						</td>
						<td>
							<?php echo showUserInfo($v['pt_inid'],$v['pt_writer'],$v); ?>
						</td>
						<?php if($pt_type == '상품평가'){ ?>
						<td><?php echo $v['pt_eval_point']; ?></td>
						<?php } ?>
						<td class="t_left">
							...
							<div class="bold">제목 : <?php echo stripslashes(htmlspecialchars(strip_tags($v['pt_title']))); ?></div>
							<div class="normal"><?php echo nl2br(stripslashes(htmlspecialchars(strip_tags($v['pt_content'])))); ?></div>
							<br>
							<?php 
										...
										<table class="table_list" style="margin-top:3px;">
											<tr>
												<td class="t_left">
													...
													<?php echo nl2br(stripslashes(htmlspecialchars(strip_tags($rpv['pt_content'])))); ?>
