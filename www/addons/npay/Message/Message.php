	...
	function &hash($hash_name, $ser = '', $enc = '') {/*{{{*/
		if (!extension_loaded('mhash')) {
			if (!Message::_inFallback($hash_name)) {
				...
				include_once LIB_PATH."Message/Hash/{$hash_name}_Fallback.php";
			...
			if (!defined($hash)) {
				...
				include_once LIB_PATH."Message/Hash/{$hash_name}.php";
		...
		if (!extension_loaded('mhash')) {
			if (!Message::_inFallback($hash_name)) {
				...
				if (in_array($hash_name, $cannot_hmac)) {
					...
					include_once LIB_PATH."Message/HMAC/{$hash_name}_Fallback.php";
			...
			if (!defined($hash) || in_array($hash_name, $cannot_hmac)) {
				...
				include_once LIB_PATH."Message/HMAC/{$hash_name}.php";
