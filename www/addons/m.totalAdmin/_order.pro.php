<?PHP
	...
	switch( $_mode ){
		...
		case "paycancel":
			...
			// $_ordernum 적용
			// 입금완료처리 :: 결제완료 => 결제대기
			include(OD_ADMIN_ROOT.'/inc.order_online.paycancel.php');
		...
		case "select_paystatus":
			...
			if(count($chk_ordernum) > 0) {
				...
				foreach($chk_ordernum as $ordernum => $val) {
					...
					# -- 저장된 결제상태가 다르고, 결제상태가 N 이고, 상태에 따른 표기가 결제대기
					if( ($row['o_paystatus'] <> $_paystatus) && $row['o_paystatus'] == 'N' && $row['o_status'] == '결제대기' ){
						...
						# -- 무통장입금확인 카운터를 증가시킨다.
						$exec_count += ($pay_status == 'N' ? 0 : 1);
						// ----- SSJ : 2020-07-01 : 결제완료/결제취소 일괄처리 -----

					} # -- end :: if( ($row['o_paystatus'] <> $_paystatus) && $row['o_paystatus'] == 'N' && $row['o_status'] == '결제대기' ){
            ...
            error_loc_msg("_order.list.php?".enc('d' , $_PVSC) , "무통장입금확인건 총 [". sizeof($chk_ordernum) ."]건 중 [".$exec_count."] 건이 정상 처리되었습니다.");// ----- SSJ : 2020-07-01 : 결제완료/결제취소 일괄처리
			break;
		# -- 2016-11-28 LCY :: 무통장다수개처리
		...
		case "cancel":
			...
			if($chk['cnt'] > 0 ) {error_msg("부분취소 하였거나 요청 중인 건이 있을 경우 주문취소를 실행할 수 없습니다.\\n\\n부분취소 요청을 삭제하신 후 취소를 진행하시거나\n\n주문상품을 부분취소하시기 바랍니다.");}
			// --- JJC : 부분취소 개선 : 2021-02-10 ---
		...
		case "mass_cancel":
            ...
            $arr_error = array(); // 오류발생 시 오류 메세지 저장
			// - 적용된 포인트, 쿠폰적용 취소 ---
			if( sizeof($chk_ordernum) >0 )	{
				...
				foreach($chk_ordernum as $k=>$v){
					...
					$chk = _MQ("SELECT COUNT(*) AS cnt FROM smart_order_product WHERE op_oordernum = '{$_ordernum}' AND op_cancel != 'N'");
					if($chk['cnt'] > 0 ) {
						$arr_error[] = '['.  $_ordernum  .'] 부분취소 하였거나 요청 중인 건이 있을 경우 주문취소를 실행할 수 없습니다';
						continue;
					}
					// --- JJC : 부분취소 개선 : 2021-02-10 ---
		...
		case "get_excel" :
			...
			$r = _MQ_assoc("select
											...
											where o_ordernum in ('".implode("','",$_ordernum_array)."')");
