<?php
/**
 * PHP Hooks Class (https://github.com/bainternet/PHP-Hooks)
 *
...
function addHook($action, $fuc) {
  global $hooks;
  $hooks->add_action($action, $fuc);
}
function actionHook($action, $arg=array()) {
  global $hooks;
  if(end(explode('.', $action)) == 'start' && empty($HookTimeArr[$action])) HookTimeArr($action);
  $hooks->do_action($action, $arg);
  if(end(explode('.', $action)) == 'end' && empty($HookTimeArr[$action])) HookTimeArr($action);
}
...
$HookTimeArr = array();
function HookTimeArr($action) {
  global $HookTimeArr;
  if(empty($HookTimeArr[$action])) $HookTimeArr[$action] = HookTime();
  return $HookTimeArr;
}
function HookTimeView() {
  global $HookTimeArr;
  $HookTimeView = $HookTimeTmp = array();

  // 각 파일의 시작과 종료 검출
  if(count($HookTimeArr) <= 0) $HookTimeArr = array();
  foreach($HookTimeArr as $k=>$v) {
    $type = end(explode('.', $k));
    if(!in_array($type, array('start', 'end'))) continue;
    $key = str_replace(array('.start', '.end'), '', $k);
    $HookTimeTmp[$key][$type] = $v;
  }

  // 걸린시간 계산
  if(count($HookTimeTmp) <= 0) $HookTimeTmp = array();
  foreach($HookTimeTmp as $k=>$v) {
    if($v['start'] && $v['end']) $HookTimeView[$k] = number_format(($v['end']?$v['end']:0)-($v['start']?$v['start']:0), 4);
  }
  return $HookTimeView;
}
$HookFileName = str_replace('.'.end(explode('.', basename($_SERVER['SCRIPT_NAME']))), '', basename($_SERVER['SCRIPT_NAME']));