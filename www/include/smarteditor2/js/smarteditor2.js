...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
	...
	$init : function(sDefaultEditingMode, elContentsField, oDimension, fOnBeforeUnload, elAppContainer){
		...
		// SSJ : textarea 높이를 에디터 min-height 로적용 : 2021-03-17
		if(this.elContentsField.style.minHeight.replace('px','')*1 > 0){
			oDimension.nMinHeight = this.elContentsField.style.minHeight.replace('px','')*1;
		}
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink, {
	...
	$ON_APPLY_HYPERLINK : function(){
		...
		if(this._validateURL(sURL)){
			...
			// -- SSJ : 네이버 에디터 링크 입력 시 새창으로 열기 추가 : 2020-08-19 ----
			var chk_obj = document.getElementsByName("_link_tarket");
			var chk_leng = chk_obj.length;
			for (i=0; i < chk_leng; i++) {
				if (chk_obj[i].checked == true) {
					sTarget = chk_obj[i].value;
				}
			}
			// -- SSJ : 네이버 에디터 링크 입력 시 새창으로 열기 추가 : 2020-08-19 ----
